...
Our methodological innovation that underpins the analyses below is a refined position-matching control procedure for isolating repetition effects in free and serial recall.
We first define repetition effects precisely, then describe how we estimate them using position-matched control lists and symmetric scoring.
Then, we introduce the datasets we analyze, highlighting the primary free and serial recall sources.
From there, we lay out the specific analyses used to test predictions of occurrence blending derived from item-based context reinstatement.
Finally, we describe the model fitting and simulations procedures used to evaluate Instance-CMR and related variants.
