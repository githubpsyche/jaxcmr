...
## Introduction
- **Research question** — What is the structure of memory for repeated experience? Frames central tension: repetition strengthens recall but each encounter has unique context
- **The spacing effect** — Anchors repetition theories; three families of mechanisms (encoding variability, deficient processing, study-phase retrieval)
- **RCT as integrative framework** — Introduces retrieved-context theory; item-context binding + contextual drift explains spacing and temporal organization
- **What makes RCT special** — Item-based reinstatement solves puzzles passive drift cannot (forward asymmetry, distractor-robust recency, prior-list intrusions)
- **The problem** — Same mechanism predicts cross-occurrence interference; contexts blend at study and retrieval
- **This chapter's contribution** — Derives and tests three diagnostic predictions; finds data don't support them; CMR produces them
- **Approach** — Holds RCT dynamics constant, manipulates reinstatement; introduces Instance-CMR variants
- **Model variants** — CMR-NoSPR, ICMR-OS, ICMR-Reinf defined; each variant's key manipulation and finding previewed
- **Road map** — Chapter structure outlined
