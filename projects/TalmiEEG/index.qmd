...
| $\theta_r$ | stop growth | Growth rate of stopping probability per output. |
| $\tau_c$ | choice sensitivity | Exponent for Luce-style competition. |
| $E_i$ | emotion indicator | 1 for emotional items, 0 for neutral items. |
| $L_i$ | centered Early LPP | List-centered Early LPP for item $i$. |
| $\kappa_E$ | emotional learning scale | Base boost to $M^{CF}_E$ learning. |
...
LPP enters the emotional learning strength through separate main and interaction terms:

$$
\ell_i = \kappa_L (L_i - \lambda_L)
$$

$$
\ell^{E}_i = \kappa_{EL} (L_i - \lambda_{EL}) E_i
$$

$$
g_i = \max\left(0,\; \kappa_E E_i + \ell_i + \ell^{E}_i\right)
$$

Here, $E_i$ is the emotion indicator and $L_i$ is the centered Early LPP value for item $i$.
The parameters $\kappa_L$ and $\lambda_L$ set the slope and centering of the LPP main effect, while $\kappa_{EL}$ and $\lambda_{EL}$ set the interaction slope and centering.
The parameter $\kappa_E$ sets the base emotional learning boost, and $g_i$ is the resulting emotional learning strength for item $i$.
...
Emotional context-to-feature learning uses the emotional context unit $c^E_i = E_i$:

$$
\Delta M^{CF_E}_{ij} = \phi_i g_i c^E_i f_i
$$
...
The parameters $\theta_s$ and $\theta_r$ set the baseline and growth rate of stopping over output positions.
If recall continues, the probability of recalling item $i$ is:

$$
P(i) = (1-P(\text{stop}))\frac{A_i^{\tau_c}}{\sum_k A_k^{\tau_c}}
$$
