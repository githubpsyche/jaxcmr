...
<!-- FIXME: consider the introdis this really the right way to start an academic paper about this topic? I worry reader should maybe have some clearer foreshadowing re what this paper will be about. -->
<!-- FIXME: i wonder the distinction provided in this paragraph and returned to later is little ill-posed since the experience of semantically similar meanings is also experientially similar. It may be more appropriate to treat experiential similarity as induced by different sources? I guess we should consider phrasing the distinction as presented by polyn2009context since we're citing them. Even then, it might be neater to avoid this thorny matter completely since, again, eCMR is compatible with any of these perspectives. -->
...
Whether this similarity is best characterised as experiential, semantic, or both may depend on the paradigm. 
eCMR is compatible with each of these perspectives, representing emotional similarity through its source context mechanism.
...
Constraining $\Phi_{\text{emot}}$ with an independent neural measure, rather than treating it as a free parameter, would provide a more principled account of individual differences in emotional memory and a stronger test of the theory — an approach consistent with the broader case for using neural data to constrain cognitive models [@turner2016more].
An excellent candidate is the late positive potential (LPP), an event-related potential (ERP) component that increases in amplitude when participants process emotionally arousing visual stimuli [@cuthbert2000brain; @schupp2000affective; @schupp2006emotion; @hajcak2010event].
In this project, our aim was to examine the suitability of the LPP for neuro-cognitive modelling of memory recall.

Changes in LPP amplitudes due to affective significance have been observed at the group level across a range of stimuli and presentation durations [@olofsson2008affective; @hajcak2010event].
However, to be useful to model an individual participant, it is also important to establish how sensitive and specific the LPP is at the level of the individual.
...
The LPP is more broadly characterised as reflecting affective stimulus significance [@hajcak2020significance; @olofsson2008affective], of which arousal is a major determinant.
<!-- FIXME: seems to undercut the previous sentence. should idea order in this paragraph be rethought? -->
These results support the use of the LPP to constrain the $\Phi_{\text{emot}}$ parameter, under the assumption that emotional arousal drives the encoding-strength advantage that $\Phi_{\text{emot}}$ captures.
<!-- FIXME: awkward wording. where does this assumption come from, anyway? -->
We note that alternative formalisations exist — for example, more recent retrieved-context models represent arousal as a source-context feature rather than as a modulator of encoding strength — which would motivate different mappings between neural measures and model parameters.
<!-- FIXME: this is a lame and lazy hedge. We don't even cite the supposed alternative formulation (which is CMR3 from Cohen et al, 2023). Integrating these details the way the draft does here just undercuts the overall framing of this introduction. We need to think more deeply about implications of these details for the framing of the paper.-->
...
Our setup extends @schupp2021case's by using a different stimulus set, a longer presentation duration (2 s vs. 150 ms), and a large independent sample (*n* = 38).
...
Although the emotional modulation of the LPP is itself reliable within individuals across testing sessions [@weinberg2021emotion], it is unknown whether this reliability extends to predicting memory differences.
As illustrated in Figure 1, eCMR predicts that larger values of $\Phi_{\text{emot}}$ yield greater recall of emotional relative to neutral stimuli.
...
This model comparison bears on the interpretation of the LPP's role in memory encoding.
If the LPP captures encoding processes that operate similarly for emotional and neutral items, the main-effect model should account for the data as well as the interaction model.
If, however, the LPP primarily reflects emotion-specific processing — such that its influence on encoding strength is larger for emotional items — the interaction model should provide the best fit.
