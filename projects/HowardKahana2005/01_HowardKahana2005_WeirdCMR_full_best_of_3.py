...
from jaxcmr.weird_cmr import BaseCMRFactory as model_factory
...
for combined_condition, condition_values in [
    ("1", [1]),
    ("2", [2]),
    ("12", [1, 2]),
]:
    for analysis in comparison_analyses:
        figure_str = f"{results['name']}_c{combined_condition}_{analysis.__name__[5:]}.png"
        ...
        lt_trial_mask = np.isin(data["condition"].flatten(), condition_values)
        ...
        _lt_trial_mask = np.isin(sim["condition"].flatten(), condition_values)
...
for combined_condition, condition_values in [
    ...
    for dataset_index, dataset in enumerate([
        ...
        for analysis in single_analyses:
            ...
            _lt_trial_mask = np.isin(dataset["condition"].flatten(), condition_values)
            _joint_trial_mask = np.logical_and(_trial_mask, _lt_trial_mask)
