...
CRU can also model confusion errors that occur during retrieval, but following the reference CRU implementation [@logan2021serial], we do not include this mechanism in our present evaluation.
...
1.  **Dynamic or Inert Feature-to-Context Memory**.
    ...
    <!-- TODO: make a clearer results statement. Does it help or doesn't it? -->
...
2.  **Use or Nonuse of a Pre-Experimental Context-to-Feature Memory**.
    ...
    By toggling this factor, we show that incorporating pre-experimental support can help CRU capture patterns of temporally distant and adjacent transitions in free and serial recall.
    <!-- TODO: does it really help in serial recall? confirm before proceeding -->
...
3.  **Primacy Gradient**.
    ...
    We don't evaluate item confusability in free recall ($g \rightarrow \infty$) and interpret CRU's $\beta_\text{dec}$ mechanism as indequate when considered alone, so for free recall evaluation we focus on assessing the effectiveness of CMR-style primacy gradient in CRU.
    ...
    In our serial recall evaluation, we have the item identification confusability mechanism on across variants, and consider whether CMR's primacy gradient either in addition to or instead of CRU's  $\beta_\text{dec}$ mechanism can further improve model performance.
...
6.  **Confusable Item Identification**.
    ...
    This factor is only evaluated in our serial recall fits, where we leave it on across all model variants to enable prediction of item intrusions. 
    Our evaluation tests whether the mechanism can be readily incorporated into CMR to help it capture patterns of intrusion and other errors in serial recall tasks and whether additionally toggling on CMR's mechani
    sms for addressing serial position effects can further improve model performance.
...
Instead, we allow models evaluated with serial recall data to repeatedly recall the same item, as well as to allow out-of-list intrusion to be recalled as well.
...
:::
<!-- TODO: update thse fits after my refined approach -->

<!-- TODO: add BIC fit table too -->
...
We first perform our evaluation using the @healey2014memory subset of the PEERS free recall dataset.
