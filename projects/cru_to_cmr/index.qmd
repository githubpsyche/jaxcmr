...
We find that selectively incorporating CMR-like features substantially improves CRU's fit to free recall, while only a subset of those features yields measurable gains for CMR on serial recall.
<!-- TODO: Check if prior sentence is right after simulation is over. -->
...
<!-- * Since we mention transposition gradients, serial position effects, and error patterns here, it makes sense to refer to these again in our serial recall results. -->
...
CRU can also model confusion errors that occur during retrieval, but following the reference CRU evaluation [@logan2021serial], we do not include this mechanism in our present evaluation.
...
A key difference is how each model obtains $\mathbf{\vec{c_i^{IN}}}$ from the active item.
...
Experimental associations are then formed during encoding whenever an item is presented, based on the simultaneous activity of item and context features $f_i$ and $c_j$, where $\gamma$ works as the learning rate:
...
Some versions of CMR replace this uniform structure with richer semantic representations [e.g., @polyn2009context; @morton2016predictive].
...
In CMR, the final state of context at the end of encoding is integrated toward the start-of-list contextual state according to a special integration rate parameter $\beta_\text{start}$ before initiating retrieval.
...
This is equivalent to fixing $\beta_\text{start} = 1$ in CMR.
...
The most complete specification of CRU also includes a mechanism where retrieved items can be incorrectly reported due to typos or other errors, but we omit this mechanism here as in its original evaluation [@logan2021serial] it was found to add complexity without consistently improving CRU's performance.
However, other work has shown that this mechanism can be useful in capturing articulatory errors in serial recall [@osth2023item].
...
Conversely, given that CMR has been shown able to address serial recall [@lohnas2024retrieved], we also ask whether CMR's added mechanisms confer any advantages over CRU in capturing the strict order of serial recall, or whether CRU's specialized mechanisms for address item confusability can provide further improvements to CMR's performance in this domain.
...
In particular, we focus on five key factors that distinguish CRU from CMR in the free and serial  recall context: (1) dynamic feature-to-context memory, (2) pre-experimental context-to-feature associations, (3) serial position scaling, (4) recall initiation, and (5) recall termination.
We also evaluate the (6) confusable item identification mechanism provided by CRU, examining how it helps CRU and could help CMR capture the strict order of serial recall.
...
**Participants.** A total of 24 participants completed multiple trials where they were presented with lists of letters and instructed to report them in the same order. 
Only a subset of trials were serial recall trials where participants began typing the letters once the list was removed.
**Stimuli**. Each list consisted of 5, 6, or 7 letters drawn at random from the English alphabet.
**Procedure**. In serial recall trials, participants were presented with the letters simultaneously on a computer screen for 1000 ms, and then the letters were removed from the screen before the participants typed their response.
...
For every trial the model

1. **encodes** the study list, updating context and associative matrices, then  
2. **scores** each recall event in sequence, adding the log-probability of the observed outcome and updating its state before the next event.

Log-likelihoods are summed across trials and maximised with differential evolution [@storn1997differential].
To compare models, we report mean and bootstrapped 95% confidence intervals for the log-likelihood and BIC scores across participants for each variant.
...
By contrast, for serial recall data, repetitions and intrusions are much more frequent and theoretically diagnostic and thus are not filtered out.
Instead, we allow models evaluated with serial recall data to repeatedly recall the same item, and when considering CRU's item confusability mechanism, we enable intrusions to be recalled as well.
...
| CRU with Free $\alpha$, $\delta$, $\phi_\text{s}$, $\phi_\text{d}$, $\beta_{\text{start}}$ | 606.05 +/- 16.56 |
| CRU with Free $\gamma$, $\phi_\text{s}$, $\phi_\text{d}$, $\beta_{\text{start}}$          | 608.00 +/- 17.00 |
...
Negative log-likelihood (±95% CI) averaged across participants for selected model variants fit to PEERS data. $\gamma$: item-to-context learning rate; $\alpha$: shared support; $\delta$: self-support; $\phi_\text{s}$: primacy scale; $\phi_\text{d}$: primacy decay; $\beta_\text{start}$: start context integration rate.
:::
<!-- TODO: LOGAN: What happens if you use AIC or BIC to penalize the fit of the more complex models? -->
...
We first perform our evaluation using the @healey2014memory free recall dataset. 
...
First, CMR leverages the primacy learning gradient mechanism modulating learning rates in the context-to-feature memory ($\phi_\text{s}$, $\phi_\text{d}$) to enhance the learning of early-list items.
Second, CMR uses a Start context integration parameter ($\beta_\text{start}$) to recover the start-of-list context.
...
The $\beta_\text{start}$ parameter is most influential at recall initiation, while the primacy learning gradient influences the primacy effect throughout retrieval.
...
In free recall, the lag-contiguity effect is bidirectional but assymmetric, favoring forward transitions over backward ones.
...
Summary statistic fits of baseline CRU (**Top**), CRU with free start context integration rate $\beta_\text{start}$ (**Middle**), and CRU freeing both start context integration rate ($\beta_\text{start}$) and primacy gradient ($\phi_\text{s}$ and $\phi_\text{d}$) parameters (**Bottom**) to @healey2014memory. **Left**: probability of recall initiation by serial position. **Middle**: conditional response probability as a function of lag. **Right**: recall probability by serial position.
:::
...
Allowing CRU to initiate retrieval with a blend of the end-of-list and start-of-list context according to a flexible start-of-recall parameter like CMR's $\beta_\text{start}$ substantially improves its ability to capture these phenomena.

@fig-shiftstart helps interpret the mechanism further by showing the impact of shifting the start context integration rate parameter $\beta_\text{start}$ on the probability of starting recall by serial position and the recall probability by serial position for CMR.
...
![](shifting/CMR_Start_Drift_Rate_Parameter_Shifting_pnr_HealeyKahana2014.png)

![](shifting/CMR_Start_Drift_Rate_Parameter_Shifting_spc_HealeyKahana2014.png)

Simulation of the impact of shifting the start context integration rate parameter $\beta_\text{start}$ on the probability of starting recall by serial position (**Left**) and the recall probability by serial position (**Right**) for CMR. Using parameters fit to @healey2014memory, $\beta_\text{start}$ is shifted from 0 to 1 in increments of 0.1, with the color of the lines indicating the value of the parameter.
...
Summary statistic fits of models to the PEERS dataset [@healey2014memory]. **Top**: CRU with free pre-experimental context-to-feature memory ($\alpha$, $\delta$), primacy gradient ($\phi_\text{s}$, $\phi_\text{d}$), and start context integration rate ($\beta_\text{start}$) parameters. **Middle**: CRU with free item-to-context learning rate ($\gamma$), primacy gradient ($\phi_\text{s}$, $\phi_\text{d}$), and start context integration rate ($\beta_\text{start}$) parameters. **Bottom**: CRU with free item-to-context learning rate ($\gamma$), pre-experimental context-to-feature memory ($\alpha$, $\delta$), primacy gradient ($\phi_\text{s}$, $\phi_\text{d}$), and start context integration rate ($\beta_\text{start}$) parameters -- equivalent to CMR. **Left**: Probability of starting recall by serial position. **Left**: Probability of starting recall by serial position. **Middle**: Conditional response probability as a function of lag. **Right**: Recall probability by serial position.
:::


In free recall, the lag-contiguity effect is bidirectional and assymmetric, with participants more likely to transition to items that were presented immediately after the just-recalled item, but also sometimes transitioning to items that were presented immediately before the just-recalled item in the original study list.
...
![](shifting/CMR_Learning_Rate_Parameter_Shifting_crp_HealeyKahana2014.png)

![](shifting/CMR_Item_Support_Parameter_Shifting_crp_HealeyKahana2014.png)
...
:::
<!-- TODO: LOGAN: The text refers to top and bottom rows of Figure 6, but there is only one row in the figure. -->
...
The strongest benefits emerge when we grant CRU the ability to initiate recall flexibly ($\beta_\text{start}$) using a mix of final and initial study context, a feature that CMR uses to balance primacy and recency effects in recall initiation [@kragel2015neural; @morton2016predictive].
...
Implementing this primacy learning gradient by allowing the strength of context-to-item associations to scale with serial position ($\phi_\text{s}, \phi_\text{d}$) yields appreciable improvements to CRU when paired with $\beta_\text{start}$, suggesting that mechanisms specific to recall initiation and encoding dynamics are crucial for capturing the serial position curve.
...
Meanwhile, CMR's greater flexibility comes at the cost of added complexity. The results here do not explore potential overfitting, but suggest that each model can be expanded or trimmed depending on task demands. 
...
<!-- TODO: It would be worth mentioning that CRU deals with repeated items (Ranschburg fits in the 2021 paper). -->
