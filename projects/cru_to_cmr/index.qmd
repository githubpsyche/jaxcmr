...
3.  **Primacy Gradient**.
    ...
    We don't evaluate item confusability in free recall ($g \rightarrow \infty$), so for this evaluation we focus on assessing the effectiveness of CMR-style primacy gradient in CRU.
...
6.  **Confusable Item Identification**.
    ...
    Our evaluation tests whether the mechanism can be readily incorporated into CMR to help it capture patterns of intrusion and other errors in serial recall tasks and whether additionally toggling on CMR's mechanisms for addressing serial position effects can further improve model performance.
