# Bridging CRU and CMR in Free and Serial Recall: A Factorial Comparison
...
Given our success in Chapter 1 demonstrating the architectural portability of retrieved-context models across composite and instance-based frameworks, we now focus exclusively on the algorithmic differences between CRU and CMR.
...
Our demonstrations in Chapter 1 showed that these two approaches can be viewed as equivalent in terms of their functional dynamics, with the linear associative memory in CMR serving a similar role to the instance-based memory in CRU.
We thus treat treat instance-based and connectionist variants of both CRU and CMR as interchangeable with respect to the present investigation.
Evaluating tasks with repeated items or strong semantic structure where these distinctions become more critical lies outside the scope of this chapter but remains a compelling direction for future research.
...
Addressing these limitations and further exploring these interactions are outside the scope of this chapter and reflect continuing challenges in modeling free recall phenomena.
...
On the other hand, while CRU's instance-based storage of item–context pairs diverges from CMR's linear associative memory, our investigation in Chapter 1 showed that these two approaches can be viewed as equivalent in terms of their functional dynamics, with the linear associative memory in CMR serving a similar role to the instance-based memory in CRU.
While both models have been shown to address item repetitions and out-of-list intrusions [e.g., @logan2021serial; @lohnas2014retrieved], it remains unclear which framework better scales across task domains to address the full range of memory phenomena.
