...
class TestLinearAssociativeMemory:
    ...
    def test_initialize_mcf(self):
        ...
        def f():
            memory1 = LinearAssociativeMcf.create(
                self.item_count, self.shared_support, self.item_support, self.choice_sensitivity)
            memory2 = LinearAssociativeMcf.create(
                self.items, self.shared_support, self.item_support, self.choice_sensitivity)
    ...
    def test_generalized_mfc(self):
        ...
        memory = LinearAssociativeMfc.create(items, .1)
    ...
    def test_dispatch_associate(self):
        ...
        mcf = LinearAssociativeMcf.create(self.item_count, self.shared_support, self.item_support)
        mfc = associate(mfc, .1, self.items[0], self.contexts[0])
        mcf = associate(mcf, .1, self.contexts[0], self.items[0])
    ...
    def test_probe_result_sum(self):
        ...
        assert jnp.sum(probe(mcf, self.contexts[0])) == 1.

    def test_choice_sensitivity(self):
        mcf = LinearAssociativeMcf.create(
                self.item_count, self.shared_support, self.item_support, self.choice_sensitivity)
        mcf2 = LinearAssociativeMcf.create(
            self.items, self.shared_support, self.item_support, 1.)
        ...
        assert(jnp.allclose(
            ...
            jnp.power(activation2, self.choice_sensitivity)/jnp.sum(
