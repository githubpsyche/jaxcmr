...
def test_initialize_mfc():
    ...
    memory2 = init_linear_mfc(item_count, learning_rate)
    memory1 = init_linear_mfc(items, learning_rate)
...
def test_initialize_mcf():
    ...
    item_support = 0.9
    items = jnp.eye(item_count, dtype=jnp.float32)
    memory1 = init_linear_mcf(item_count, shared_support, item_support)
    memory2 = init_linear_mcf(items, shared_support, item_support)
    assert jnp.array_equal(memory1.state, memory2.state)
...
def test_generalized_mfc():
    ...
    memory = init_linear_mfc(items, learning_rate)
...
def test_dispatch_associate():
    ...
    mfc = init_linear_mfc(item_count, learning_rate)
    mcf = init_linear_mcf(item_count, shared_support, item_support)
    mfc = associate(mfc, .1, items[0], contexts[0])
    mcf = associate(mcf, .1, contexts[0], items[0])
...
def test_shape_selective_dispatch():
    ...
    learning_rate = 0.1
    items = jnp.eye(item_count, dtype=jnp.float32)
    memory = init_linear_mfc(item_count, learning_rate)
