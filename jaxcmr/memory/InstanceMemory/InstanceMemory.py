...
Another popular class of memory models are instance-based memory models, which store and retrieve specific instances of items and contexts instead of learning association weights.

Here we provide a simple implementation of an instance-based memory based on the MINERVA 2 model [@hintzman1984minerva, @hintzman1986schema, @hintzman1988judgments] that associates patterns by concatenating them into a single vector and storing them in a stack of discrete memory traces. 
...
The memory returns just the portion of the activation vector corresponding to feature dimensions for the target pattern type.
