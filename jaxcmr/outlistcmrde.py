"""
Rename soon: OutlistCMR-DE. This works better than the original Context CMR-DE model.
"""
...
class CMR(Pytree):
    ...
    def experience_item(self, item_index: Int_) -> "CMR":
        ...
        #! We use a special integration process if self.recallable.at[item_index] is already True
        #! (which correspods to to the item having already been encoded before)
        new_context = lax.cond(
            self.recallable[item_index],
        ...
        #! We associate the item with current context state instead of new_context in this implementation
        return self.replace(
            context=new_context,
            mfc=self.mfc.associate(item, self.context.state, self.mfc_learning_rate),
            mcf=self.mcf.associate(self.context.state, item, self.mcf_learning_rate),
...
def BaseCMR(list_length: int, parameters: Mapping[str, Float_]) -> CMR:
    context = TemporalContext.init_expanded(list_length)
...
def InstanceCMR(list_length: int, parameters: Mapping[str, Float_]) -> CMR:
    ...
    mfc = InstanceMemory.init_mfc(
        ...
        parameters["learning_rate"],
        parameters["mfc_choice_sensitivity"],
        1.0,  # parameters["mfc_trace_sensitivity"],
    )
