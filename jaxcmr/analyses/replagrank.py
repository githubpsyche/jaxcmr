"""Repetition lag-rank temporal factor score.

Computes per-presentation temporal factor scores for transitions FROM
repeated items, decomposed by which presentation of the source item
the transition originates from. Parallels ``repcrp`` but produces a
scalar factor per presentation index instead of a CRP curve.

Notes
-----
- ``should_tabulate`` restricts ranking to transitions FROM items with
  at least two study positions separated by more than ``min_lag``.
- For each qualifying transition, the absolute lag from each study
  position of the previous item is ranked among all available absolute
  lags from that same position. Results are accumulated separately per
  presentation index into ``rank_sum`` and ``transition_count`` arrays
  of shape ``(size,)``.
- The cumulative-bin ranking approach matches ``lagrank.Tabulation``:
  available absolute lags are collapsed into boolean bins, then ranked
  by cumulative count. A score of 1.0 = nearest neighbor, 0.0 = furthest.
...
class RepLagRankTabulation(Pytree):
    """Per-presentation lag-rank tabulator for repeated items."""
    ...
    def should_tabulate(self) -> Bool:
        """Return True when previous item is repeated with spacing > min_lag."""
        return (
            (self.previous_positions.size > 1)
            & ((self.previous_positions[-1] - self.previous_positions[-2]) > self.min_lag)
        )

    def _tabulate(self, recall: Int_) -> "RepLagRankTabulation":
    ...
    def conditional_tabulate(self, recall: Int_) -> "RepLagRankTabulation":
        """Rank only when previous item qualifies."""
        return lax.cond(
    ...
    def tabulate(self, recall: Int_) -> "RepLagRankTabulation":
...
def tabulate_trial(
    ...
    """Tabulate per-presentation lag-rank for a single trial.
    ...
    """
    init = RepLagRankTabulation(presentation, trial[0], min_lag, size)
...
def subject_rep_lagrank(
    ...
    subject_values = apply_by_subject(
        dataset,
        trial_mask,
        jit(replagrank, static_argnames=("min_lag", "size")),
        min_lag=min_lag,
        size=size,
    )
...
@dataclass
class RepLagRankTestResult:
    """Results from a per-presentation lag-rank test."""
...
def test_rep_lagrank_vs_control(
    observed: np.ndarray,
    control: np.ndarray,
) -> dict[str, RepLagRankTestResult]:
    ...
    results: dict[str, RepLagRankTestResult] = {}
    ...
    for rep_idx in range(size):
        ...
        results[label] = RepLagRankTestResult(
...
def plot_rep_lagrank(
    ...
    for d_idx, data in enumerate(datasets):
        factors = subject_rep_lagrank(data, trial_masks[d_idx], min_lag=min_lag, size=size)
