...
Estimates the probability that two study positions separated by lag ``d`` are
both recalled on the same trial, ignoring recall order.
"""
...
def _corec_counts(
    ...
    """Returns co-recall numerators and denominators for a single trial.
    ...
    Returns:
      (actual_pairs, possible_pairs): Positive-lag co-recall counts and the
      number of available pairs for each lag.
    """
...
def corec_by_lag(dataset: RecallDataset) -> Float[Array, " lags"]:
    """Returns CoRec(d) for positive lags.
...
def plot_corec_by_lag(
    ...
    """Returns Matplotlib ``Axes`` with co-recall curves for each dataset.
