"""Order error rate analyses for serial recall."""
...
def trial_order_error_rate(
    ...
    """Returns a Boolean vector marking order errors for each study position.

    Args:
      recalls: Recalled items for one trial. Shape (recall_positions,).
      presentations: Studied items for the trial. Shape (study_positions,).
...
def order_error_rate(
    ...
    """Returns the mean order-error rate per study position across trials.

    Args:
      recalls: Trial by recall position array; 1-indexed items, 0 for omissions.
      presentations: Trial by study position array; 1-indexed, 0 for padding.
      list_length: Number of studied items in each trial.
      size: Maximum number of study positions an item can occupy.
    """
