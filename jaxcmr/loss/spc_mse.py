...
class MemorySearchSpcMseLoss:
    """SPC-based MSE loss for a dataset and model factory."""
    ...
    def __call__(
        ...
        def loss_for_one_sample(x_row: jnp.ndarray) -> Float[Array, ""]:
            ...
            return lax.cond(
                use_base_loss,
                lambda _: self.base_spc_mse(trial_indices, parameters),
                lambda _: self.present_and_predict_spc_mse(
                    trial_indices, parameters
                ),
                operand=None,
            )
...
MemorySearchSpcMseFnGenerator = MemorySearchSpcMseLoss
