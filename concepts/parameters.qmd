...
A complete parameter dictionary for BaseCMR:
...
For models with semantic similarity (e.g., `AdditiveSematicCMR`):
...
When fitting parameters to data:

1. **Fix some parameters** based on experimental design or prior knowledge
2. **Fit others** using differential evolution or other optimization
3. Use `ScipyDE` with bounds appropriate for each parameter

See [Fitting Tutorial](../tutorials/fitting.qmd) for a complete workflow.
